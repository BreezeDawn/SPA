...
def setup_log(name):
    ...
    _ = '%(log_color)s[%(asctime)s] [%(filename)s:%(lineno)d] [%(module)s:%(funcName)s] [%(levelname)s]- %(message)s'
    formatter = colorlog.ColoredFormatter(_, log_colors=log_colors_config)
